...
'''
    ...
    graph_generator.set_parameters(add_conn_threshold=0.53)
    graph_generator.set_parameters(remove_conn_threshold=0.47)
    graph_generator.set_parameters(add_node_threshold=0.54)
    ...
    for i in range(25):
        hedwig.critical_info(f"Experiment WITH HIDDEN number: {i}")
    ...
    for i in range(25):
        hedwig.critical_info(f"Experiment NO HIDDEN number: {i}")
        best, average = evo.run_evolution(callback)
        baseline_bests.append(best)
        baseline_averages.append(average)

    hedwig.critical_info(f'''
        NO HIDDEN EXPERIMENT (with no hidden)
    ...
    hedwig.critical_info(f'''
        NO HIDDEN EXPERIMENT (with hidden)
