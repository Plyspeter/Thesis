...
'''
    ...
    evo_config["fitness"]["func"] = "fitness_penalize_trivial"
    evo.set_parameters(**evo_config)
    ...
    for i in range(25):
        hedwig.critical_info(f"Experiment penalize trivial number: {i}")
    ...
    evo_config["fitness"]["func"] = "fitness_nca"
    evo.set_parameters(**evo_config)
    
    for i in range(25):
        hedwig.critical_info(f"Experiment NO penalize trivial number: {i}")
        best, average = evo.run_evolution(callback)
        baseline_bests.append(best)
        baseline_averages.append(average)

    hedwig.critical_info(f'''
        TRIVIAL PENALTY EXPERIMENT
